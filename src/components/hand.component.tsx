...
export function HandComponent (props: { cards: SelectableCard[]; selectCard: any }): React.JSX.Element {
  const [cards, setCards] = useState([...props.cards]);

  const selectCard = (card): void => {
  ...
  return (
    <div className="hand">
      {props.cards?.map(c => (<SelectableCardComponent key={c.number} card={c} selectCard={selectCard}/>))}
    </div>
  )
}
